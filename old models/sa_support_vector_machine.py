...
clf = SVC(kernel='linear').fit(xtrain_bow,ytrain)
...
print(tabulate([["Bag-of-words","Support Vector Machines",f1_score(yvalid, prediction_int)]], headers=["Building model with","algorithm","accuracy"]))
...
print(tabulate([["TF-IDF features","Support Vector Machines",f1_score(yvalid, prediction_int)]], headers=["Building model with","algorithm","accuracy"]))
...
print('\n')
print(tabulate([["on test set","Support Vector Machines",f1_score(yvalid, prediction_int)]], headers=["Prediction and Evaluation","algorithm","accuracy"]))

test['label'] = test_pred
submission = test[['tweet','label']]
col_names = ['tweet','label']
submission.to_csv('Results_from_SVM_labeled_test.csv', index=False, index_label=None) # writing data to a CSV file
print('\n')
